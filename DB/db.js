...
var connection = (function () {
  ...
  mysqlConnectionInstance.prototype.configure = function (
    host,
    user,
    pswd,
    db,
  ...
  mysqlConnectionInstance.prototype.getConnection = function () {
    return this.configure(process.env.DB_HOST || 'localhost', process.env.DB_USER || "root", process.env.DB_PASSWORD || "@Aa25126631", process.env.DB_NAME || `kanban_saas`, 3306);
  };
  ...
  mysqlConnectionInstance.prototype.query = async function (con, queryString) {
    var queryResult = new Promise(async (resolve, reject) => {
      con.query(
        queryString,
        await function (err, rows, fields) {
          if (err) {
            reject(err);
          } else {
            resolve(rows);
  ...
  mysqlConnectionInstance.prototype.queryByArray = async function (
    con,
    queryString,
    param,
    callback
  ...
  mysqlConnectionInstance.prototype.checkTableExists = async function (
    con,
    schemaName,
    tableName
  ) {
